...
fn run(ctx: *app.Ctx, a: cli.Args(Spec)) anyerror!u8 {
    ...
    const old_live = edit.liveTarget(ctx.alloc, context.paths.home, context.cwd, old_name) catch |e| switch (e) {
        error.OutOfMemory => return e,
        else => |f| return edit.reportTarget(ctx.err, "mox mv", old_name, f),
    };
    const new_live = edit.liveTarget(ctx.alloc, context.paths.home, context.cwd, new_name) catch |e| switch (e) {
        error.OutOfMemory => return e,
    ...
    const tree = mox.source.tree.walkDiag(ctx.alloc, ctx.io, src_dir, context.paths.home, &walk_diag) catch |e| switch (e) {
        ...
        => {
            try ctx.err.print("mox mv: ownership declaration: {s}: {s}\n", .{
        ...
        => {
            try ctx.err.print("mox mv: attributes.toml: {s}: {s}\n", .{
