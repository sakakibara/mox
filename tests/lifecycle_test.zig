...
test "edit --apply: the edit reaches the live file; without it the live file stays stale" {
    ...
    const editor = try FakeEditor.installWriter(a, io, &tmp, root, "first");
    ...
    // --apply carries THIS edit the rest of the way, in one command: the
    // editor now writes different bytes, so a stale apply would show.
    _ = try FakeEditor.installWriter(a, io, &tmp, root, "second");
...
test "status: a run probing both axes prints tool then env under one blank-line lead-in" {
    const io = std.testing.io;
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();
    var arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    ...
    const h = try testutil.setup(a, io, &tmp, .{
        .create_repo_src = true,
        .extra_env = &.{
            .{ .name = "PATH", .value = bin_dir },
            .{ .name = "MOX_STATUS_PROBE_VAR", .value = "1" },
        },
    });
    try writeRepo(io, &tmp, "repo/src/.has-fd", "# mox: when tool=fd\nyes\n# mox: end\n");
    try writeRepo(io, &tmp, "repo/src/.has-hedrr", "# mox: when tool=hedrr\nyes\n# mox: end\n");
    ...
    try std.testing.expect(std.mem.indexOf(
        u8,
        r.out,
        "\ntool probed: fd present, hedrr ABSENT\n" ++
            "env probed: MOX_STATUS_PROBE_TYPO ABSENT, MOX_STATUS_PROBE_VAR present\n",
    ) != null);
}
...
test "rollback: no id restores the newest snapshot and names it; none at all is refused" {
    ...
    try std.testing.expectEqualStrings("export A=1\n", try read(io, a, try h.liveOf(".zshrc")));
    // A second snapshot, a second later so its id sorts after the first.
    io.sleep(.fromMilliseconds(1100), .awake) catch {};
    try Io.Dir.cwd().writeFile(io, .{ .sub_path = try h.liveOf(".zshrc"), .data = "second hand edit\n" });
    _ = try h.run(&.{ "mox", "apply", "--overwrite" });
    ...
    try std.testing.expectEqualStrings("second hand edit\n", try read(io, a, try h.liveOf(".zshrc")));
}
