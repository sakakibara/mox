...
keymap tables in a config the program otherwise rewrites at runtime. Declaring
`own = ["<key-path>", ...]` for a target in `.mox/attributes.toml` makes mox
manage only those key subtrees: compose, drift protection, and commit work on
the owned paths with full mox semantics, and mox never changes a byte outside
them -- verified before every write, and refused outright when the live file
cannot be patched provably. Drift follows the whole-file rule, per owned path:
the live owned content is compared against what mox last applied, so a change
inside an owned path -- the program's or yours -- is surfaced, never silently
overwritten, and taking ownership of existing content (first apply, or a path
newly added to `own`) adopts it only when it already matches the source. An
optional `check` attribute names a repo script that must accept each candidate
before it is written. Onboard a file with `mox add --own`; the walkthrough is in
[docs/usage.md](docs/usage.md#a-file-a-program-also-writes) and the attribute
reference in [docs/dsl.md](docs/dsl.md#file-attributes).
...
| `add <path>` | Start managing a live file as a base file in `src/`. A path matching a repo ignore rule (`.moxignore` / `.mox/ignore`) is refused (`--force` overrides). `--own <key-path>` (repeatable) takes partial ownership of a structured file instead: the named subtrees are extracted into the source (comments inside them survive) and the `own` list is recorded in `.mox/attributes.toml`; `--own-absent <key-path>` declares a key mox enforces as absent. A plain `add` of a target that has an `own` declaration is refused |
...
| `mv <old> <new>` | Rename a managed file's source (base file and its `.d/` overlay dir) so the live target changes on the next apply. The old source is copied into the timestamped trash first (recoverable); its `.mox/attributes.toml` entry (mode, symlink, seed-once, own/check) is carried to the new name, and a partial target's owned record is re-keyed with it -- the old live file keeps its owned content, like any orphaned live file. Takes the lock |
...
| `status` | Show each managed file's state: `clean`, `OUTDATED`, `DRIFT`, `MISSING`, `GATED`, `ERROR`. A partially owned file is classified on its owned subtree only, so program writes outside it never surface. Exits 1 if any file is `OUTDATED`, `DRIFT`, `MISSING`, or `ERROR` |
...
| `doctor` | Health report: source files not tracked by git, source modes git cannot carry that are not yet in `.mox/attributes.toml` (lost on clone), sources that compose to nothing under every configuration (a contradictory or mistyped whole-file gate), an `own` declaration whose target has no source, and malformed state (provenance). `--rebuild-provenance` recomposes and re-records every tracked file's provenance (partial targets keep no line provenance and are skipped); `--rebuild-coupling` rescans source tokens and rewrites the stored coupling graph under `<state>/coupling/`; `--fix` performs the safe rebuilds. Mutating runs take the lock; exits 1 while problems remain |
